...
std::optional<R> IsObjectComparable(const ObjectHolder& lhs, const ObjectHolder& rhs, F comparator) {
    T* lhs_ptr = lhs.TryAs<T>();
    T* rhs_ptr = rhs.TryAs<T>();
    if (rhs_ptr && lhs_ptr) {
        return comparator(lhs_ptr->GetValue(), rhs_ptr->GetValue());
    }
    return {};
}    

template<typename F>
std::optional<bool> CompareValuableObjectIfPossible(const ObjectHolder& lhs, const ObjectHolder& rhs, F comparator) {
    auto result = IsObjectComparable<Number, bool>(lhs, rhs, comparator);
    if (result) {
        return result.value();
    }
    result = IsObjectComparable<String, bool>(lhs, rhs, comparator);
    if (result) {
        return result.value();
    }
    result = IsObjectComparable<Bool, bool>(lhs, rhs, comparator);
    if (result) {
        return result.value();
    }
    return {};
